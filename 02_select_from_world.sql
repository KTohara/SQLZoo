...
FROM world
WHERE continent LIKE UPPER('%south america%');
...
SELECT name,
       ROUND(population/1000000, 2) AS population_per_million,
       ROUND(gdp/1000000000, 2) AS gdp_per_billion
FROM world
WHERE continent LIKE UPPER('%south america%');
...
SELECT name, capital, LEN(name) as name_length
FROM world
