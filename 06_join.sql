...
JOIN
  game ON game.id = goal.matchid
WHERE
  'GER' IN (game.team1, game.team2) AND goal.teamid <> 'GER';
...
SELECT
  game.stadium,
  COUNT(*) AS total_goals_per_stadium
FROM
  game
JOIN
  goal ON goal.matchid = game.id
...
SELECT
  ...
  game.team1,
  SUM(CASE WHEN
        goal.teamid = game.team1 THEN 1
      ELSE
        0
      END) AS score1,
  game.team2,
  SUM(CASE WHEN
        goal.teamid = game.team2 THEN 1
      ELSE
        0
      END) AS score2
FROM
